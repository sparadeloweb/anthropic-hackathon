...
const posts = await getPosts(user.id); // This one depends on user
```

### Key Pattern: Suspense Streaming
```tsx
...
// Called multiple times in the same request, only fetches once
```

### Key Pattern: Minimize Serialization
```tsx
// BAD: Passing entire object to client
<ClientComponent data={hugeObject} />

// GOOD: Pass only what client needs
<ClientComponent name={hugeObject.name} count={hugeObject.items.length} />
```
...
### Key Pattern: No Inline Components
```tsx
// BAD: Remounts on every parent render
function Parent() {
  const InlineChild = () => <div>child</div>;
  return <InlineChild />;
}
...
For the complete expanded documentation with full code examples for all 70 rules:
```bash
npx skills add https://github.com/vercel-labs/agent-skills --skill react-best-practices --global
```
